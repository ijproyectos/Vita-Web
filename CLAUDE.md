...
**Netlify está vivo**: sitio `vitappweb` (`site_id` `3c956a4d-b077-4ef4-a6e0-b6cef204ed44`, cuenta `ijsociety-exe`), conectado a `ijproyectos/Vita-Web` rama `main` (auto-deploy). `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` y `GROQ_API_KEY` cargadas en Netlify (scope "All") — el chat con vita, la extracción automática de estudios y el modo voz ya tienen lo que necesitan en producción. `OPENAI_API_KEY` quedó cargada pero sin uso real (la cuenta de OpenAI del usuario no tiene créditos — ver decisión de usar Groq para el modo voz en "Chat con vita" arriba); el módulo de voz corre contra Groq.

**Pendiente, acción del usuario:**
- [ ] Agregar `GROQ_API_KEY` a `.env.local` (Claude no tiene permiso para escribir ese archivo — se cargó en Netlify pero falta en local).
