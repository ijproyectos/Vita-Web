...
import { crearMedicamentoParaElderAction, type EstadoAccion } from "../actions";
...
// Calca app/app/agregar-medicamento/formulario.tsx — misma UI, pero la
// action de submit crea el medicamento para `elderId` (el cuidador actúa
// sobre datos de otra persona, ver crearMedicamentoParaElderAction).
export function FormularioAgregarMedicamentoElder({ elderId }: { elderId: string }) {
  ...
  // Sin toast de éxito: crearMedicamentoParaElderAction redirige del lado
  // del servidor en el camino feliz (mismo criterio que
  // aceptarInvitacionAction/AceptarInvitacionForm) — el `redirect()`
  // interrumpe antes de que este `await` devuelva "success", así que solo
  // el estado de error llega a manejarse acá.
  const accion = crearMedicamentoParaElderAction.bind(null, elderId);
